...
void printManipulateStudentInterface(int idx)
{
    ...
    while (input)
    {
        ...
        cout << "Input your choice: ";
        if (!(cin >> input)) { // 🟢 If input is not an integer, clear input and retry
            cin.clear();
            cin.ignore(numeric_limits<streamsize>::max(), '\n');
            cout << "Invalid input! Please enter a number." << endl;
            system("pause");
...
void printCreateNewClassroomInterface()
{
    ...
    cout << "_______________________________________________________" << endl;
    cout << "INPUT NUMBER OF STUDENT IN NEW CLASSROOM: " << endl;
...
void printSaveClassroomToFileInterface()
{
    ...
    do
    {
        ...
        if(filename.length()<8){
            cout<<"File name must be longer 8 characters."<<endl;
            continue;
        }
